...
namespace render_graph::core
{
    namespace
    {
        ...
        {
            if (requested == queue_class::compute && !environment.queues.compute) return queue_class::graphics;
            if (requested == queue_class::copy && !environment.queues.copy) return queue_class::graphics;
            return requested;
        }
        ...
        void append_access(compiler_state& state, access_event event)
        {
            for (auto& existing : state.accesses)
            {
                ...
                if (event.kind == resource_kind::buffer)
                {
                    ...
                    if (before.bytes != after.bytes) before.bytes = {};
                }
                else
                {
                    ...
                    before.usage = before.usage | after.usage;
                    if (before.subresource != after.subresource) before.subresource = {};
                }
                return;
            }
        ...
        {
            return {
                .size = std::max<uint64_t>(1, static_cast<uint64_t>(desc.extent.width) * desc.extent.height *
                                                 desc.extent.depth * desc.array_layers * 4),
                .alignment = 256,
                                                   ...
                                                   resource_kind kind) noexcept
        {
            ...
            if (before.queue != after.queue) intents |= synchronization_intent::queue_ownership;
            return intents;
        }
    ...
    {
        if (!state.request || !state.request->frame)
        {
        ...
        if (frame.passes.size() + (state.request->inject_stable_upload_pass ? 1u : 0u) >
            state.request->environment.limits.max_passes)
        ...
        for (uint32_t index = 0; index < frame.passes.size(); ++index)
        {
            ...
            if (!valid_range(pass.buffer_accesses, frame.buffer_accesses.size()) ||
                !valid_range(pass.image_accesses, frame.image_accesses.size()) ||
                !valid_range(pass.attachments, frame.attachments.size()) ||
                !valid_range(pass.buffer_copies, frame.buffer_copies.size()) ||
            ...
            if ((pass.kind == pass_kind::raster &&
                ...
                (pass.kind == pass_kind::copy &&
                 (pass.attachments.count != 0 || pass.dispatches.count != 0 ||
                  pass.indexed_indirect_draws.count != 0)))
                fail(state, compile_error_code::unsupported_feature,
                     "frame commands are incompatible with the pass kind", pass_handle{index});
    ...
    bool build_resource_versions(compiler_state& state)
    {
        ...
        if (request.inject_stable_upload_pass)
        {
            ...
            plan.upload_buffer = plan.resources.buffer_metas.add(
                "UploadArena", desc, resource_lifetime_class::imported, hash_resource_desc(desc), true);
            state.buffer_contracts.emplace_back();
            plan.passes.push_back({.name = "UploadPass", .kind = pass_kind::copy,
                                   .queue = available_queue(queue_class::copy, request.environment),
                                   .source_pass = invalid_source_pass, .backend_upload = true});
        }
        ...
        for (uint32_t index = 0; index < frame.resources.size(); ++index)
        {
                ...
                resource.source == frame_resource_source::transient_buffer)
            {
                ...
                if (imported && (!request.descriptions.describe_buffer ||
                    !request.descriptions.describe_buffer(request.descriptions.state, resource.buffer, desc)))
                {
                    fail(state, compile_error_code::backend_failure, "persistent buffer handle is stale",
                         invalid_pass, resource_kind::buffer, index);
                    continue;
                }
                desc.lifetime = imported ? resource_lifetime_class::imported : resource_lifetime_class::transient;
                const auto validation = request.validation.validate_buffer
                    ? request.validation.validate_buffer(request.validation.state, desc)
                    : validate_resource_desc(nullptr, desc);
                if (!validation)
                {
                    fail(state, compile_error_code::unsupported_feature, validation.message,
                         invalid_pass, resource_kind::buffer, index);
                    continue;
                }
                plan.frame_buffers[index] = plan.resources.buffer_metas.add(
                    std::string(resource.name), desc, desc.lifetime, hash_resource_desc(desc), imported);
                state.buffer_contracts.emplace_back();
            }
            else
            {
                image_desc desc = resource.image_description;
                bool imported = resource.source == frame_resource_source::persistent_image ||
                                resource.source == frame_resource_source::swapchain_image;
                if (resource.source == frame_resource_source::persistent_image &&
                    (!request.descriptions.describe_image ||
                     !request.descriptions.describe_image(request.descriptions.state, resource.image, desc)))
                {
                    fail(state, compile_error_code::backend_failure, "persistent image handle is stale",
                         invalid_pass, resource_kind::image, index);
                    continue;
                }
                ...
                else desc.lifetime = imported ? resource_lifetime_class::imported
                                              : resource_lifetime_class::transient;
                const auto validation = request.validation.validate_image
                ...
                plan.frame_images[index] = plan.resources.image_metas.add(
                    std::string(resource.name), desc, desc.lifetime, hash_resource_desc(desc), imported);
        ...
        for (uint32_t source = 0; source < frame.passes.size(); ++source)
        {
            ...
            pass.raster.area = {.width = request.environment.extent.width,
                                .height = request.environment.extent.height};
        ...
        if (request.inject_stable_upload_pass)
        {
            append_access(state, access_event{
                .pass = pass_handle{0}, .kind = resource_kind::buffer,
                .logical = static_cast<resource_handle>(plan.upload_buffer), .access = access_type::read,
                .state = buffer_access_desc{.usage = buffer_usage::TRANSFER_SRC,
                                            .domain = pipeline_domain::copy,
                                            .queue = plan.passes[0].queue},
            });
            for (uint32_t index = 0; index < frame.resources.size(); ++index)
            {
                if (frame.resources[index].source != frame_resource_source::persistent_buffer ||
                    plan.frame_buffers[index] == invalid_buffer)
                    continue;
                ...
                append_access(state, access_event{
                    .pass = pass_handle{0}, .kind = resource_kind::buffer,
                    .logical = static_cast<resource_handle>(logical), .access = access_type::write,
                    .state = buffer_access_desc{.usage = buffer_usage::TRANSFER_DST,
                                                .domain = pipeline_domain::copy,
                                                .queue = plan.passes[0].queue},
                });
            }
        ...
        for (uint32_t source = 0; source < frame.passes.size(); ++source)
        {
            ...
            for (uint32_t at = row.buffer_accesses.begin; at < row.buffer_accesses.begin + row.buffer_accesses.count; ++at)
            {
                ...
                if (access.resource.index >= frame.resources.size() ||
                    plan.frame_buffers[access.resource.index] == invalid_buffer)
                {
                    fail(state, compile_error_code::buffer_read_out_of_range,
                         "buffer access references a non-buffer resource", pass, resource_kind::buffer,
                         access.resource.index);
                    continue;
                }
                append_access(state, access_event{
                    .pass = pass, .kind = resource_kind::buffer,
                    .logical = static_cast<resource_handle>(plan.frame_buffers[access.resource.index]),
                    .access = access.access,
                    .state = buffer_access_desc{.usage = access.usage, .domain = pass_domain,
                                                .queue = pass_queue, .bytes = access.range},
                });
            }
            for (uint32_t at = row.image_accesses.begin; at < row.image_accesses.begin + row.image_accesses.count; ++at)
            {
                ...
                if (access.resource.index >= frame.resources.size() ||
                    plan.frame_images[access.resource.index] == invalid_image)
                {
                    fail(state, compile_error_code::image_read_out_of_range,
                         "image access references a non-image resource", pass, resource_kind::image,
                         access.resource.index);
                    continue;
                }
                append_access(state, access_event{
                    .pass = pass, .kind = resource_kind::image,
                    .logical = static_cast<resource_handle>(plan.frame_images[access.resource.index]),
                    .access = access.access,
                    .state = image_access_desc{.usage = access.usage, .domain = pass_domain,
                                               .queue = pass_queue, .subresource = access.range},
                });
            }
            for (uint32_t at = row.attachments.begin; at < row.attachments.begin + row.attachments.count; ++at)
            {
                ...
                if (attachment.resource.index >= frame.resources.size() ||
                    plan.frame_images[attachment.resource.index] == invalid_image)
                {
                    fail(state, compile_error_code::raster_attachment_mismatch,
                         "attachment references a non-image resource", pass, resource_kind::image,
                         attachment.resource.index);
                    continue;
                }
                const auto logical = plan.frame_images[attachment.resource.index];
                const auto usage = attachment.kind == frame_attachment_kind::color
                    ? image_usage::COLOR_ATTACHMENT : image_usage::DEPTH_STENCIL_ATTACHMENT;
                append_access(state, access_event{
                    .pass = pass, .kind = resource_kind::image,
                    .logical = static_cast<resource_handle>(logical),
                    .access = attachment.load == attachment_load_op::load
                        ? access_type::read_write : access_type::write,
                    .state = image_access_desc{.usage = usage, .domain = pipeline_domain::graphics,
                                               .queue = pass_queue},
                });
            ...
            if (row.kind == pass_kind::raster && plan.passes[pass].raster.colors.empty() &&
                !plan.passes[pass].raster.has_depth_stencil)
                fail(state, compile_error_code::raster_pass_has_no_attachments,
                     "raster pass has no attachments", pass);
        }
        ...
        for (uint32_t index = 0; index < frame.resources.size(); ++index)
        {
            if (plan.frame_images[index] != invalid_image)
            {
                ...
                if (frame.resources[index].source == frame_resource_source::swapchain_image)
                {
                    ...
                    contract.initial_state = {.usage = request.environment.swapchain_initialized
                                                          ? image_usage::PRESENT : image_usage::NONE,
                                              .domain = pipeline_domain::graphics};
                    contract.initial_access = access_type::read;
                    contract.initial_contents = request.environment.swapchain_initialized
                        ? contents_policy::preserve : contents_policy::discard;
                    contract.has_final_state = true;
                    contract.final_state = {.usage = image_usage::PRESENT,
                                            .domain = pipeline_domain::graphics};
                }
            }
        }
    ...
    {
        core::build_dependency_dag(state.accesses,
    ...
    bool compile_lifetimes(compiler_state& state)
    {
        ...
        for (const auto& event : state.accesses)
        {
            ...
            if (event.kind == resource_kind::image)
                update(plan.lifetimes.image_first_used_pass, plan.lifetimes.image_last_used_pass,
                       image_first_order, image_last_order);
            else update(plan.lifetimes.buffer_first_used_pass, plan.lifetimes.buffer_last_used_pass,
        ...
        for (resource_handle logical = 0; logical < image_count; ++logical)
        {
            ...
            if (desc.lifetime == resource_lifetime_class::transient &&
                desc.aliasing != aliasing_policy::forbidden)
            {
                for (resource_handle candidate = 0; candidate < logical; ++candidate)
                {
                    if (plan.resources.image_metas.descs[candidate] == desc &&
                        image_first_order[logical] != std::numeric_limits<uint32_t>::max() &&
            ...
            if (reuse != invalid_resource)
            {
                ...
                physical.alias_handoffs.push_back({resource_kind::image, reuse, logical,
                    physical.handle_to_image_memory_block[reuse], plan.lifetimes.image_first_used_pass[logical]});
            }
            else
            {
                physical.handle_to_physical_img_id[logical] =
                    static_cast<resource_handle>(physical.physical_image_meta.size());
                physical.physical_image_meta.push_back(logical);
                if (!plan.resources.image_metas.is_imported[logical])
                {
                    const auto requirements = state.request->allocations.image_requirements
                        ? state.request->allocations.image_requirements(state.request->allocations.state, desc)
                        : default_image_requirements(desc);
                    physical.handle_to_image_memory_block[logical] =
                        static_cast<resource_handle>(physical.image_memory_blocks.size());
        ...
        for (resource_handle logical = 0; logical < buffer_count; ++logical)
        {
                ...
                desc.aliasing != aliasing_policy::forbidden)
            {
                for (resource_handle candidate = 0; candidate < logical; ++candidate)
                {
                    if (plan.resources.buffer_metas.descs[candidate] == desc &&
                        buffer_first_order[logical] != std::numeric_limits<uint32_t>::max() &&
            ...
            if (reuse != invalid_resource)
            {
                ...
                physical.alias_handoffs.push_back({resource_kind::buffer, reuse, logical,
                    physical.handle_to_buffer_memory_block[reuse], plan.lifetimes.buffer_first_used_pass[logical]});
            }
            else
            {
                physical.handle_to_physical_buf_id[logical] =
                    static_cast<resource_handle>(physical.physical_buffer_meta.size());
                physical.physical_buffer_meta.push_back(logical);
                if (!plan.resources.buffer_metas.is_imported[logical])
                {
                    const auto requirements = state.request->allocations.buffer_requirements
                        ? state.request->allocations.buffer_requirements(state.request->allocations.state, desc)
                        : default_buffer_requirements(desc);
                    physical.handle_to_buffer_memory_block[logical] =
                        static_cast<resource_handle>(physical.buffer_memory_blocks.size());
    ...
    bool compile_synchronization(compiler_state& state)
    {
        ...
        struct last_state { bool valid = false; abstract_resource_state state{}; pass_handle pass = invalid_pass; };
        ...
        for (const auto pass : plan.scheduled_passes)
        {
            for (const auto& event : state.accesses)
            {
                ...
                if (previous.valid) before = previous.state;
                else if (event.kind == resource_kind::image)
                {
                    ...
                    {
                        access_event initial{.kind = resource_kind::image, .access = contract.initial_access,
                ...
                else
                {
                    ...
                    if (contract.has_initial_state)
                    {
                        access_event initial{.kind = resource_kind::buffer, .access = contract.initial_access,
                                             .state = contract.initial_state};
                        before = abstract_state(initial);
                        have_before = true;
                    }
                ...
                {
                    const auto physical = event.kind == resource_kind::image
                        ? plan.physical_resources.handle_to_physical_img_id[event.logical]
                        : plan.physical_resources.handle_to_physical_buf_id[event.logical];
                    const auto block = event.kind == resource_kind::image
                        ? plan.physical_resources.handle_to_image_memory_block[event.logical]
                    ...
                    prologues[pass].push_back({
                        ...
                        .phase = before.queue == after.queue ? synchronization_phase::full
                                                            : synchronization_phase::acquire,
                        .intents = intents,
                        .kind = event.kind,
        ...
        for (const auto& handoff : plan.physical_resources.alias_handoffs)
        {
            ...
            for (const auto& event : state.accesses)
                ...
                { after = abstract_state(event); break; }
            prologues[handoff.at_pass].push_back({
                ...
                .physical = handoff.kind == resource_kind::image
                    ? plan.physical_resources.handle_to_physical_img_id[handoff.next]
        ...
        for (resource_handle logical = 0; logical < images.size(); ++logical)
        {
            ...
            if (!contract.has_final_state || !images[logical].valid) continue;
            access_event final_event{.kind = resource_kind::image, .access = contract.final_access,
            ...
            if (intents == synchronization_intent::none) continue;
            epilogue.push_back({
    ...
    bool compile_submissions(compiler_state& state)
    {
        ...
        for (const auto pass : plan.scheduled_passes)
        {
            ...
            if (submissions.batches.empty() || submissions.batches.back().queue != queue)
            {
                ...
                submissions.batches.push_back({.handle = handle, .queue = queue,
                                                .signal_value = static_cast<uint64_t>(handle.value) + 1});
            }
        ...
        for (pass_handle source = 0; source < state.dag.outgoing.size(); ++source)
        {
            for (const auto destination : state.dag.outgoing[source])
            {
                ...
                if (std::ranges::none_of(batch.waits, [&](const timeline_wait& wait)
                    { return wait.source_batch == source_batch; }))
        ...
        {
            if (!has_intent(op.intents, synchronization_intent::queue_ownership) ||
                op.source_pass == invalid_pass || op.pass == invalid_pass)
                continue;
    ...
    void publish_compiled_plan(compiler_state& state)
    {
        ...
        hash = combine(hash, (static_cast<uint64_t>(state.request->environment.extent.width) << 32) |
                             state.request->environment.extent.height);
        hash = combine(hash, static_cast<uint64_t>(state.request->environment.color_format));
        hash = combine(hash, state.request->environment.swapchain_initialized);
        for (const auto& desc : plan.resources.image_metas.descs) hash = combine(hash, hash_resource_desc(desc));
        for (const auto& desc : plan.resources.buffer_metas.descs) hash = combine(hash, hash_resource_desc(desc));
        for (const auto& pass : plan.passes)
        {
            hash = combine(hash, static_cast<uint64_t>(pass.kind));
            hash = combine(hash, static_cast<uint64_t>(pass.queue));
            for (const auto value : pass.name) hash = combine(hash, static_cast<uint8_t>(value));
            hash = combine(hash, pass.raster.layer_count);
            hash = combine(hash, pass.raster.has_depth_stencil);
            ...
            for (const auto& attachment : pass.raster.colors) hash_attachment(attachment);
            if (pass.raster.has_depth_stencil) hash_attachment(pass.raster.depth_stencil);
        }
        for (const auto& event : state.accesses)
        {
...
namespace render_graph
{
    graph_compile_output compile_graph(const graph_compile_request& request)
    {
        ...
        if (!core::validate_recipe(state)) return std::move(state.output);
        if (!core::build_resource_versions(state)) return std::move(state.output);
        if (!core::build_dependency_dag(state)) return std::move(state.output);
        if (!core::schedule_passes(state)) return std::move(state.output);
        if (!core::compile_lifetimes(state)) return std::move(state.output);
        if (!core::compile_synchronization(state)) return std::move(state.output);
        if (!core::compile_submissions(state)) return std::move(state.output);
